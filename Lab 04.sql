...
insert into country (name, capital, continent, region, surface_area, independence_year, population, life_expectancy, gdp, local_name, government_form, political_system, gcr, code_iso)
values
  ('Коста-Ріка', 'Сан-Хосе', 'Північна Америка', 'Центральна Америка', 51100, 1821, 3703869, 79.3, 22270, 'Коста-Ріка', 'Парламентська республіка', 'Унітарна держава', 288, 'CR'),
  ('Нікарагуа', 'Манагуа', 'Північна Америка', 'Центральна Америка', 130000, 1838, 5074000, 68.7, 1988, 'Нікарагуа', 'Президентська республіка', 'Унітарна держава', 97, 'NI'),
  ('Гондурас', 'Тегусігальпа', 'Північна Америка', 'Центральна Америка', 112090, 1838, 6485000, 69.9, 5333, 'Гондурас', 'Президентська республіка', 'Унітарна держава', 39, 'HN'),
  ('Беліз', 'Бельмопан', 'Північна Америка', 'Центральна Америка', 22696, 1981, 241000, 70.9, 630, 'Беліз', 'Парламентська монархія', 'Унітарна держава', NULL, 'BZ'),
  ('Сальвадор', 'Сан-Сальвадор', 'Північна Америка', 'Центральна Америка', 21041, 1841, 6276000, 69.7, 11863, 'Сальвадор', 'Президентська республіка', 'Унітарна держава', 35, 'SV'),
  ('Гватемала', 'Гватемала', 'Північна Америка', 'Центральна Америка', 108889, 1821, 11385000, 66.2, 19008, 'Гватемала', 'Президентська республіка', 'Унітарна держава', 35, 'GT'),
  ('Панама', 'Панама', 'Північна Америка', 'Центральна Америка', 75517, 1903, 2856000, 75.5, 9131, 'Панама', 'Президентська республіка', 'Унітарна держава', 59, 'PA'),
  ('Норвегія', 'Осло', 'Європа', 'Північна Європа', 323877, 1905, 4478500, 78.7, 145895, 'Норвегія', 'Парламентська конституційна монархія', 'Унітарна держава', 3837, 'NO'),
  ('Фінляндія', 'Гельсінкі', 'Європа', 'Північна Європа', 338145, 1917, 5171300, 77.4, 121914, 'Фінляндія', 'Парламентська республіка', 'Унітарна держава', 4225, 'FI'),
  ('Самоа', 'Апіа', 'Австралія та Океанія', 'Полінезія', 2832, 1962, 187820, 75.2, 4231, 'Самоа', 'Конституційна монархія', 'Унітарна держава', 185, 'WS'),
  ('Фіджі', 'Сува', 'Австралія та Океанія', 'Меланезія', 18274, 1970, 817000, 67.9, 1536, 'Фіджі', 'Парламентська республіка', 'Унітарна держава', 1386, 'FJ'),
  ('Тонга', 'Наукі', 'Австралія та Океанія', 'Полінезія', 748, 1970, 99000, 66.3, 146, 'Тонга', 'Парламентське королівство', 'Унітарна держава', NULL, 'TO'),
  ('Кірибаті', 'Тарава', 'Австралія та Океанія', 'Мікронезія', 726, 1979, 83000, 59.8, 46, 'Кірибаті', 'Республіка', 'Унітарна держава', NULL, 'KI'),
  ('Вануату', 'Порт-Віла', 'Австралія та Океанія', 'Меланезія', 12189, 1980, 190000, 60.6, 261, 'Вануату', 'Республіка', 'Унітарна держава', NULL, 'VU');
...
INSERT INTO city (name_city, district, population_city, country_id_country)
VALUES
  ('Торонто', 'Онтаріо', 2731571, (select id_country from country where lower(name) = 'канада')),
...
INSERT INTO country_language (language, is_official, percentage, country_id_country)
VALUES
  ('Англійська', true, 52.0, (select id_country from country where name = 'Індія')),
...
-- Task 1
select name, capital from country where region = 'Південна Америка';

-- Task 2
select name, country_language.language from country
join country_language on country.id_country = country_language.country_id_country
where lower(language) in ('арабська', 'японська');

-- Task 3
select name, city.name_city from country
left join city on country.id_country = city.country_id_country
where continent = 'Австралія та Океанія' and city.id_city is null;
...
having count(language) = (
    ...
    from (
        select count(language) as official_language_count
        from country_language
        where is_official = true
...
select name
from country
where id_country in (select country_id_country from city group by country_id_country having count(distinct name_city) = 3);

-- Task 9
(select name, 'Три офіційні мови' as description
from country
where id_country in (select country_id_country from country_language group by country_id_country having count(language) = 3))
union
(select name, 'Три адміністративні одиниці' as description
from country
where id_country in (select country_id_country from city group by country_id_country having count(distinct district) = 3));
...
-- Task 12
select name, language, is_official from country
join country_language cl on country.id_country = cl.country_id_country
where is_official = true and language in (
    select language from country_language
    join public.country c on country_language.country_id_country = c.id_country
    where c.name = 'Канада' and is_official = true);
